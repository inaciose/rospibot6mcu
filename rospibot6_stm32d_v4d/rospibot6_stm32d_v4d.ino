...
#define ENCODER_RIGHT_FUNCTIONA encoderRightCounterA
#define ENCODER_RIGHT_FUNCTIONB encoderRightCounterB
...
void encoderRightCounterA() {
  // look for a low-to-high on channel A
  if (digitalRead(ENCODER_RIGHT_PINA) == HIGH) {
    // check channel B to see which way encoder is turning
    if (digitalRead(ENCODER_RIGHT_PINB) == LOW) {
      encoderRightPulses = encoderRightPulses + 1;         // CW
      // pid
    ...
    // check channel B to see which way encoder is turning
    if (digitalRead(ENCODER_RIGHT_PINB) == HIGH) {
      encoderRightPulses = encoderRightPulses + 1;          // CW
      // pid
      encoderRightPulsesSpeedPID++;
      encoderRightPulsesSteeringPID++;
      ...
      encoderRightPulses = encoderRightPulses - 1;          // CCW
      // pid
      encoderRightPulsesSpeedPID--;
      encoderRightPulsesSteeringPID--;
    }
...
void encoderRightCounterB() {
  ...
  // Look for a high-to-low on channel B
    ...
    // check channel B to see which way encoder is turning
    if (digitalRead(ENCODER_RIGHT_PINA) == LOW) {
      encoderRightPulses = encoderRightPulses + 1;          // CW
      encoderRightPulsesSpeedPID++;
      encoderRightPulsesSteeringPID++;
    }
...
void setup() {
  ...
  attachInterrupt(ENCODER_RIGHT_PINA, ENCODER_RIGHT_FUNCTIONA, ENCODER_RIGHT_SIGNAL);
  attachInterrupt(ENCODER_RIGHT_PINB, ENCODER_RIGHT_FUNCTIONB, ENCODER_RIGHT_SIGNAL);
